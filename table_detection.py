...
def drawIoUResult(img, sample_annotations, boundedBox):
    ...
    for bbox in sample_annotations:
        ...
        for bbox_pending in boundedBox:
            if (((bbox[0] <= bbox_pending[0]) and (bbox_pending[0] <= bbox[2]) and (bbox[1] <= bbox_pending[1]) and (
                    bbox_pending[1] <= bbox[3])) or (
                    (bbox_pending[0] <= bbox[2]) and (bbox[2] <= bbox_pending[2]) and (bbox_pending[1] <= bbox[1]) and (
                    bbox[1] <= bbox_pending[3])) or (
                    (bbox_pending[0] <= bbox[0]) and (bbox[0] <= bbox_pending[2]) and (bbox_pending[1] <= bbox[1]) and (
                    bbox[1] <= bbox_pending[3])) or (
                    (bbox[0] <= bbox_pending[2]) and (bbox_pending[2] <= bbox[2]) and (bbox[1] <= bbox_pending[1]) and (
                    bbox_pending[1] <= bbox[3]))):
    ...
    print("Precision: ", round((true_cnt / ground_truth_cnt), 2))
    global Total
    Total = Total + round((true_cnt / ground_truth_cnt), 2)
    print("------------")
...
def tableDetection(img_path, xml_path):
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (round(y1 / (doc + 2) / 6), round(x1 / (ngang + 2) / 4)))  # kernel width on the left, kernal height on the right
...
for i in os.listdir('./forms/'):  # image store in "forms" folder
    if i.endswith('.png') or i.endswith('.PNG') or i.endswith('.jpg') or i.endswith('.JPG'):
        tableDetection(os.path.splitext('./forms/' + i)[0] + '.png', os.path.splitext('./forms/' + i)[
            0] + '.xml')  # os.path.splitext use for split file name out of extension


print ("Average precision: ", Total/ (len(os.listdir('./forms/'))/2))